...
最近更新：**R165 深度并发安全审查 — 修复 async_loader 3 处并发问题** — 深度审查异步资源加载器的线程安全，发现并修复 3 处并发问题。**R165-A**（CONCURRENCY）：完成队列容量 `ASYNC_QUEUE_SIZE=256` 小于最大请求数 `ASYNC_MAX_REQUESTS=1024`，当 256+ 个完成项在消费前入队时，环形缓冲区静默覆盖旧条目，导致陈旧/损坏数据。修复：将 `ASYNC_QUEUE_SIZE` 提升至 1024 匹配最大请求数。**R165-B/C**（CONCURRENCY）：worker 线程在 I/O 完成后使用 `atomic_store` + `enqueue_completion` 两步操作设置最终状态，与主线程的 `async_loader_cancel`（CAS `ASSET_LOADING → ASSET_CANCELLED`）存在 TOCTOU 竞态。worker 可能在 cancel 后覆盖 `ASSET_CANCELLED` 为 `ASSET_READY`/`ASSET_FAILED`，导致已取消请求的回调仍然触发。修复：引入 `async_finalize()` 函数，使用 `atomic_compare_exchange_strong` 原子地将 `ASSET_LOADING` 转换为最终状态，若检测到 `ASSET_CANCELLED` 则释放数据并跳过完成入队。range load 路径（R165-C）和 full file read 路径（R165-B）共 8 处状态转换全部统一使用 `async_finalize()`。framework 代码（3 个 C++ 桩文件）和平台 demo 代码（Linux XCB/OpenGL + Windows D2D/D3D）审查确认无问题。编译验证：Vulkan 100% + GL 100%。测试验证：Vulkan 23/23 + GL 全部通过。总计 430 处修复。
