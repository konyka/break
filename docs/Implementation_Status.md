...
最近更新：**R164 工具代码与 shader 深层审查 — 修复 10 处 I/O 返回值与整数溢出问题** — 首次系统性审查 R102-R163 未覆盖的领域：131 个 shader 文件、31 个测试文件、CMake 构建系统（590 行）、framework 代码、工具代码（packer.c 318 行, verify_pak.c 168 行）。**R164-A**（ROBUSTNESS）：`packer.c` 中数据偏移累加使用 `u32 offset`，当总打包数据超过 4GB 时 u32 回绕，产生静默损坏的 PAK 文件（data_offset 字段错误）。修复：使用 `u64 total_offset` 累加器并检查 `> 0xFFFFFFFFull` 溢出，超限时报错退出。**R164-B**（ROBUSTNESS）：`packer.c` 中 6 处 `fwrite` 调用未检查返回值（magic/version/entry_count/name_size/entries/names），磁盘满时产生截断的 PAK 文件但报告成功。修复：添加返回值检查，失败时报错并退出。**R164-C/D**（ROBUSTNESS）：`packer.c` 的 `write_file_data` 函数中 Windows 路径和 Linux 路径的 `fwrite` 调用未检查返回值。修复：添加返回值检查 + 资源清理 + 报错。**R164-E**（ROBUSTNESS）：`verify_pak.c` 中 `fread` 调用未检查返回值，I/O 错误时使用未初始化数据比较。修复：添加返回值检查 + 资源清理。shader 文件审查确认所有除法有适当守卫（`ray.y > 0.01` 条件守卫、`clip.w <= 0.0` 近平面守卫、`max_life > 0.001` 除零守卫）。测试文件审查确认测试逻辑正确、边界覆盖充分。CMake 构建系统审查确认编译标志完善（-Wall -Wextra -Werror -pedantic）、第三方库隔离正确（glad -Wno-pedantic, lua -w）。编译验证：Vulkan 100% + GL 100%。测试验证：Vulkan 23/23 + GL 30/30 通过。总计 427 处修复。
...
- **审计总计（R129-R164）**：**427 处**全量加固，涵盖 calloc/malloc NULL 检查、Vulkan VkResult 全路径检查、fseek/fwrite/fread/fclose 返回值检查（含工具代码）、strncpy null 终止、snprintf 截断检查、usize→u32/int 截断防护、线程创建检查、数学除零防护、窗口尺寸 0 防护、stbi_load_from_memory 截断检查、mipmap 级别尺寸乘法溢出防护、Vulkan push constant 越界防护、delta_time 钳制防护、Vulkan swapchain 获取图像错误处理防护、Vulkan framebuffer 创建/访问 NULL 解引用防护、场景图 parent_index 越界读防护、视锥剔除缓冲区溢出防护、mip 链生成栈溢出与偏移截断防护、BVH 构建 OOM 崩溃防护、g_node_vis/node_spheres 固定数组越界读防护、任务系统 calloc 失败 NULL 解引用与 pool_count 越界读防护、RHI 资源池耗尽 slot 覆盖损坏防护、VFS PAK entry_count 乘法溢出防护、内存分配器 usize 加法溢出防护、VFS PAK name_offset 越界读防护、解码管线 usize→u32 截断防护、地形 grid_size=0 u32 下溢堆缓冲区溢出防护、LOD level_count 超限越界读防护、LOD 屏幕尺寸策略 inv_bias 逻辑错误修复、packer.c data_offset u32 溢出防护、packer.c fwrite 返回值检查、verify_pak.c fread 返回值检查。
