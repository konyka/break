...
最近更新：**R206 体积光视空间光照 + DOF focus_range — 修复 2 处** — **R206-A**（CORRECTNESS）：`volumetric` 在视空间射线与世界空间 `sun_dir` 上做 dot，相机旋转时散射错误；用已上传的 `u_vol_view` 将光向变换到视空间。**R206-B**（CORRECTNESS）：DOF 推送 `u_dof_range` 但 CoC 用 near/far，`focus_range` 无效；改为 `abs(depth-focus)/range`。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 535 处修复。
