...
最近更新：**R111 GPU 剔除初始化验证 + 热重载路径终止修复** — **R111-1**（ROBUSTNESS）：`gpucull_init` 创建 3 个 GPU 缓冲区（`object_ssbo`、`visible_ssbo`、`count_buf`）后未验证有效性就设置 `ready = true`。若任一缓冲区创建失败，后续操作使用无效 RHI 句柄，导致崩溃或未定义行为。引擎中同类初始化函数（`indirect_draw_init`、`gpucull_init_unified`、`occlusion_cull_init`）均有完整验证，仅此函数遗漏。修复：添加三缓冲区有效性检查，失败时调用 `gpucull_shutdown` 清理并返回 false。**R111-2**（ROBUSTNESS）：`hotreload_pipeline_init` 未 `memset` 结构体就 `strncpy` 路径，当着色器路径 ≥255 字节时不保证 null 终止。`hotreload_texture_init` 正确使用了 `memset`。修复：入口添加 `memset(hr, 0, sizeof(*hr))`。审查确认 12 个子系统（UTF-8 解码、BVH 构建/遍历/查询、间接绘制、GPU 剔除统一管线、遮挡剔除、IBL、天空盒、接触阴影、SSS、ImGui、热重载纹理、后期处理着色器读取）无需修复。23/23 测试通过。
