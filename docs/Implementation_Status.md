...
最近更新：**R195 GL offscreen 可采样 depth + Hi-Z 生成后恢复 mip — 修复 2 处** — **R195-A**（CORRECTNESS）：GL offscreen 深度为 renderbuffer 且未设 `depth_tex`，Hi-Z/SSAO 等整段跳过；改为 D32 纹理并注册 handle。**R195-B**（CORRECTNESS）：Hi-Z 末尾 `bind_texture_mip` 钳最后一级，unified 跳过 dispatch 时不恢复；生成结束再 `bind_texture_compute`。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 513 处修复。
