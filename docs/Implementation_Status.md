...
最近更新：**R129 全量 calloc/malloc NULL 检查审计 — RHI 后端 7 处遗漏修复** — 对全代码库执行 `grep -rn 'calloc|malloc'` 全量审计，发现 R125+R128 遗漏了“辅助 FBO”函数中的 7 处 calloc：GL `rhi_cubemap_depth_fbo_create`（2 处）、VK `rhi_mrt_fbo_create`（3 处）、VK `rhi_cubemap_depth_fbo_create`（2 处）。修复：calloc 移到 rhi_alloc_slot 之前，检查 NULL，返回空结构体。全量审计确认 17 个子系统全安全：core/platform/renderer/asset/ecs/physics/audio/vfs/asset/font/task/script/network/scene_serial/main.c 均有 NULL 检查。VK pipeline/shader malloc 有检查。VK 初始化路径 8 处 calloc 为已知限制。RHI 后端全量 calloc/malloc NULL 检查审计完成。23/23 测试通过。
此前：**R128 第十轮深度审查 — GL 后端 6 处 calloc NULL 检查遗漏修复** — **R128-1**（ROBUSTNESS）：R125 修复了 rhi_gl.c 中 8 处 calloc NULL 检查，但遗漏了 6 处：`rhi_cubemap_create`（GLTextureData，GL 纹理已创建）、`rhi_offscreen_fbo_create_fmt`（GLFBOData）、`rhi_gpu_timer_create`（RHIGPUTimer）、`rhi_mrt_fbo_create`（GLMRTFBOData + 色纹理循环 GLTextureData + 深度纹理 GLTextureData）。修复：calloc 移到 rhi_alloc_slot 之前，检查 NULL，清理 GL 资源（glDeleteTextures）或返回空结构体。审查确认 VK 后端 rhi_offscreen_fbo_create_fmt 3 处已被 R125 修复。23/23 测试通过。
