...
最近更新：**R194 GL/VK sampler mip 过滤对齐 — 修复 2 处** — **R194-A**（CORRECTNESS）：GL sampler `MIN_FILTER` 无 MIPMAP，`textureLod` 恒采 mip0；改为 MIPMAP 变体并设 `MAX_LEVEL`/`mip_levels`。**R194-B**（CORRECTNESS）：VK `mipmapMode` 恒 LINEAR，NEAREST Hi-Z 层间误混合；按 `min_filter` 选择。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 511 处修复。
