...
最近更新：**R236 延迟路径 Hi-Z/后处理深度源修正 — 修复 1 处** — **R236-A**（CORRECTNESS）：`RENDER_PATH_DEFERRED` 下前向场景 Pass 被跳过、延迟光照管线 `depth_write_disable`，故 `scene_fbo.depth_tex` 从不写入；而 Hi-Z 遮挡与全部深度型后处理（SSAO/接触阴影/体积光/SSR/SSGI/TAA/运动模糊/DoF/SSS/God Rays/debug_viz/upscale）仍采样 `scene_fbo.depth_tex`，读到空/陈旧深度。真实几何深度在 G-Buffer 的 `deferred.gbuf_depth`。新增 `scene_depth` 选择器：延迟且已初始化时用 `gbuf_depth`，否则用 `scene_fbo.depth_tex`（前向路径字节等价，golden 不受影响）。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 31/31（含 golden-image 回归）。总计 594 处修复。
