...
最近更新：**R212 Hi-Z 窗口深度比较 + vol/cs/lf 默认关闭 — 修复 2 处** — **R212-A**（CORRECTNESS）：`unified_cull`/`occlusion_cull` 用 NDC z 对比 Hi-Z 窗口深度 `[0,1]`，遮挡判断偏移；改为 `*0.5+0.5`，并修正球视锥近平面 `-1`。**R212-B**（PERF）：`vol`/`cs`/`lf` 默认开但 FBO 从未合成；默认关闭避免半分辨率空跑。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 547 处修复。
