...
最近更新：**R85 着色器效率优化(第三轮)：blinn_phong albedo缓存 + skybox循环不变量 + pow→mul批量替换 + F_Schlick去重** — **R85-1**（HIGH GPU）：blinn_phong_clustered 着色器在方向光/点光源循环内每次迭代重新采样 `u_albedo` 纹理，N+M 个光源导致 N+M 次冗余纹理采样。缓存到局部变量 `albedo`，循环内直接复用。同时将 `pow(x,32.0)` 替换为 5 次平方链。**R85-2**（HIGH GPU）：skybox 着色器云层 24 次迭代循环中 `normalize(pos)` 冗余（`pos = ray * t`，`ray` 已归一化），`dot(normalize(pos), sun) = dot(ray, sun) = cos_sun` 是循环不变量。替换为 `max(cos_sun, 0.0)`。同时将 `pow(x,3.0)` 和 `pow(x,1.5)` 替换为乘法/sqrt。**R85-3**（MEDIUM GPU）：跨 18 个着色器批量替换所有 `pow()` 整数/半整数指数调用：`pow(x,32.0)`→5 次平方、`pow(x,3.0)`→3 次乘法、`pow(x,1.5)`→`x*sqrt(x)`、`pow(x,5.0)`→5 次乘法（含 brdf_lut.comp 1024 次循环）。**R85-4**（MEDIUM GPU）：pbr_clustered 着色器中 `F_Schlick(max(dot(N,V),0.0), F0)` 在 IBL 路径下被调用两次（一次计算 `kD`，一次计算 `F_env`），但 `kD` 在 IBL 路径中为死代码。合并为单次调用，`#else` 路径复用 `kD_env`。**回归**：全部 **23/23** 测试通过。
此前：**R84 着色器效率优化(第二轮)：POM uniform门控 + SSGI循环不变量提升 + shadow_test提升 + F_Schlick pow→mul** — R84-1 POM门控、R84-2 SSGI循环不变量提升、R84-3 shadow_test提升、R84-4 F_Schlick pow→mul。23/23 测试通过。
