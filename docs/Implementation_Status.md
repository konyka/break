...
最近更新：**R231 unified_cull Hi-Z unit + clear_color 语义 — 修复 2 处** — **R231-A**（CORRECTNESS）：`gpucull_dispatch_unified` 把 Hi-Z 绑到 unit 0，GL `unified_cull.comp` 为 `binding=4`；Hi-Z 遮挡错误。GL 改绑 unit 4。**R231-B**（CORRECTNESS）：GL `clear_color` 附带清 depth，与 VK 仅清 color 不一致；改为只清 color，forward 显式 `clear_depth`。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 585 处修复。
