...
最近更新：**R222 GL SSR/SSGI sampler binding — 修复 2 处** — **R222-A**（CORRECTNESS）：`ssr.frag` 双 sampler 默认 unit 0，深度追踪失效；补 binding 0/1 对齐 `bind_textures_multi`/VK。**R222-B**（CORRECTNESS）：`ssgi.frag` 同理（depth@0 color@1）。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 567 处修复。
