...
最近更新：**R159 全面验证审查（无新问题）** — 对 R129-R158 的 412 处修复进行验证轮次。审查 26 个源文件覆盖所有子系统：音频（audio.c, audio_stream.c）、动画（animation.c, skeleton.c）、脚本（script.c, script_lua.c）、地形（terrain.c）、粒子（particles.c）、间接绘制（indirect_draw.c）、物理（physics.c, bvh.c）、命令缓冲区（cmd_buffer.c）、异步加载器（async_loader.c）、解码管线（decode_pipeline.c）、字体（font.c）、UTF-8（utf8.c）、手柄（gamepad_linux.c）、ECS（ecs.c）、网络（network.c, packet.c）、场景序列化（scene_serial.c）、glTF 资产加载（asset.c）、RHI 句柄管理（rhi.c）、内存分配器（alloc.c, pool.c）、任务系统（task.c）。验证所有 R129-R158 修复完好：R155 g_node_vis 越界读防护 7 处、R156 calloc 失败+pool_count 越界读 9 处、R157 RHI 池耗尽 abort+VFS PAK 溢出 2 处、R158 usize 溢出 3 处。未发现新安全问题。总计 412 处修复，30/31 测试通过。
