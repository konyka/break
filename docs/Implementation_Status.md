...
最近更新：**R69 审查加固 + 性能优化** — 栈数组补漏 + Deferred采样器修复 + terrain预计算 + 除法转乘法。**R69-1**：`light_system_cull` 10个局部数组（~16.5KB）改 `static`，与R65-R66修复模式一致。**R69-2**：Deferred GL路径点阴影采样器从 `_gbuf_sampler`(NEAREST)修复为 `_linear_sampler`(LINEAR)，消除R68引入的阴影边缘锯齿回归；`_linear_sampler` 在 `deferred_init` 创建/`deferred_destroy` 销毁。**R69-3**：帧循环 `pl_pos[256]`/`pl_rad[256]`(~4KB) 改 `static`。**R69-4**：`point_shadow_update` 的 `PSCandidate cand[256]`(~2KB) 改 `static`。**R69-5**：`deferred.h` 残留 `#include "point_shadow.h"` 移除；`psc_light_idx` 添加 NULL 防护（传 NULL 时写 0xFFFFFFFF 哨兵值）。**R69-6**：`Terrain` 结构体新增 `inv_scale`/`inv_nm1` 预计算字段；`terrain_get_height`/`modify_height`/`flatten`/`erode`/`noise_stamp` 中 `x/t->scale` → `x*t->inv_scale`，`(f32)(grid_size-1)` → `inv_nm1`，消除每帧热路径除法。**R69-7**：`light_system_cull` 逐光源 `radius/-view_z` 除法改为预计算倒数乘法。**回归**：test_math 45/45、test_camera_frustum 24/24。
