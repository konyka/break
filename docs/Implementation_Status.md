...
最近更新：**R181 shadow pass 状态机 + 静态 mesh DEVICE_LOCAL — 修复 2 处** — **R181-A**（CORRECTNESS）：`unbind/bind_shadow_map` End 后未清 `render_pass_active`，`!framebuffers` 早退留下假 active；与 offscreen unbind 对齐并清 `pass_suspended`。**R181-B**（PERF）：带 `initial_data` 的 VERTEX/INDEX 改 `DEVICE_LOCAL` + staging 上传；动态 VBO（font 等无 initial_data）仍 HOST_VISIBLE。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 485 处修复。
