...
最近更新：**R77 纹理缓存提升修复正确性bug + 间接绘制buffer缓存 + occlusion_cull指针化** — **R77-1**：`g_tex_cache`/`g_sam_cache`/`g_active_unit` 原为 `gl_bind_tex_unit` 的函数静态变量。`rhi_cmd_bind_texel_buffers` 和 `rhi_cmd_bind_texture_mip` 绕过缓存直接调 `glActiveTexture`+`glBindTexture`，导致缓存不同步——后续 `gl_bind_tex_unit` 误判缓存命中跳过 `glActiveTexture`，将纹理绑定到错误的纹理单元（正确性 bug）。将缓存提升为文件作用域，两个函数改为通过缓存路径绑定。`rhi_cmd_bind_texture_mip` 额外缓存 mip level 避免重复 `glTexParameteri` 调用（每帧 ~50 次冗余调用）。**R77-2**：`rhi_cmd_draw_indexed_indirect`/`rhi_cmd_draw_indexed_indirect_count` 每次调用后无条件解绑 `GL_DRAW_INDIRECT_BUFFER`/`GL_PARAMETER_BUFFER_ARB`，多组循环中产生 bind→draw→unbind→bind→draw→unbind 周期。添加 `g_gl_indirect_buf`/`g_gl_param_buf` 缓存，跳过重复绑定，移除尾部解绑。**R77-3**：`occlusion_cull_dispatch` 按值传 `Mat4`（64B/次），改为 `const Mat4*` 指针，与 R74-3 `mat4_vec4` 模式一致。**回归**：全部 **23/23** 测试通过。
