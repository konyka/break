...
最近更新：**R209 god rays 方向投影 + 体积雾世界高度 — 修复 2 处** — **R209-A**（CORRECTNESS）：god rays 用有限远点 `-100*sun_dir` 乘 VP（含平移），相机平移时太阳 UV 漂移；改为方向 `w=0` 投影。**R209-B**（CORRECTNESS）：volumetric 高度雾用 view-space `pos.y`，点头/平移时跟着相机；`inverse(u_vol_view)` 取世界 Y。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 541 处修复。
