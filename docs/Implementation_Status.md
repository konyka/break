...
最近更新：**R202 水面阴影采样器 + 点光阴影 push 映射 — 修复 2 处** — **R202-A**（CORRECTNESS）：`water_render` 传 `(RHISampler){0,0}` 致 VK 跳过描述符绑定、水面无阴影；改为自有 sampler。**R202-B**（CORRECTNESS）：点光 `u_mvp`/`u_light_pos`/`u_far_plane` 未映射致 cubemap 深度错误；补 `is_shadow_depth` 分支。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 527 处修复。
