...
最近更新：**R184 font 双槽 + 粒子 SSBO DEVICE_LOCAL — 修复 2 处** — **R184-A**（CORRECTNESS）：font 单槽 VBO 每帧 host 写与上一帧 VS 竞态；改为 `vbo[2]` + frame_index。**R184-B**（PERF）：粒子 STORAGE 热路径仍 HOST_VISIBLE；带 `initial_data` 的 GPU-only STORAGE 改 DEVICE_LOCAL，粒子三缓冲用零初始化创建。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 491 处修复。
