...
最近更新：**R175 粒子 cull GPU 清零 + mip upload 布局跟踪 — 修复 2 处** — **R175-A**（CORRECTNESS）：`particles_cull` 用 host `rhi_buffer_update` 清 `instanceCount`，与在途 draw_indirect/cull 竞态；改为 init 写死 header，每帧 `rhi_cmd_fill_buffer` 仅清 instanceCount。**R175-B**（CORRECTNESS）：`rhi_texture_upload_mip` 硬编码 oldLayout=READ_ONLY，R174 数据路径高层 mip 仍为 UNDEFINED；改用 `mip_layout[]`。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 471 处修复。
