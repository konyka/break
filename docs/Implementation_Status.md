...
最近更新：**R76 逆索引越界修复 + glViewport缓存统一 + barrier批处理 + skeleton_upload条件化** — **R76-1**：R75-2 逆索引构建中，当材质数 > `MEGA_MAX_MAT_GROUPS`(64) 时 `cmd_mat_group[ci]` 可为 64，直接用作 `group_counts[64]` 下标越界写栈缓冲。添加边界检查 `if (g < mega_buf.mat_group_count)` 跳过溢出命令。**R76-2**：`glViewport` 缓存 `g_gl_vp` 原为 `gl_cmd_set_viewport` 的静态局部变量，9 处 FBO/shadow 绑定函数绕过缓存直接调用 `glViewport`，导致每帧 ~25 次冗余 `glViewport` 调用 + 缓存不同步风险。将缓存提升为文件作用域，所有调用点统一通过 `gl_set_viewport_cached` 函数。**R76-3**：`indirect_draw_compact` 每组材质结束时发出 `rhi_cmd_memory_barrier`，一个 pass 内 G 组产生 G 次 barrier。新增 `indirect_draw_compact_no_barrier` 函数，将全部 4 处多组循环改为两阶段：先循环所有组的 upload + compact_no_barrier，发出单个 `rhi_cmd_memory_barrier`，再循环所有组的 bind + execute。将 G 次 barrier/pass 降为 1 次。**R76-4**：`skeleton_upload` + skinned pipeline bind + 7 个 uniform set 在 `skinned_mesh_count > 0` 检查之前无条件执行，当场景无蒙皮网格且无 fallback VBO 时全部浪费。包裹在 `if (skinned_mesh_count > 0 || rhi_handle_valid(skinned_vbo))` 条件内。**回归**：全部 **23/23** 测试通过。
