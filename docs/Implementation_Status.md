...
最近更新：**R221 GL upscale/volumetric sampler binding — 修复 2 处** — **R221-A**（CORRECTNESS）：默认 50% render scale 下 `upscale.frag` 三 sampler 默认 unit 0，depth/history 失效；补 binding 0/1/2 对齐 material bind（src/depth/history）与 VK。**R221-B**（CORRECTNESS）：`volumetric.frag` 同理；补 binding 0/1。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 565 处修复。
