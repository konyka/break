...
最近更新：**R197 upscale history 真复制 + 去掉 debug_viz/lens 中间 unbind — 修复 2 处** — **R197-A**（CORRECTNESS）：Pass 2 误再跑 TSR（`u_ups_sharp=0` 只关锐化）污染 history；新增 `u_ups_copy_only` 原样 blit，并补 VK `u_ups_*` push 映射（此前 loc 恒 -1）。**R197-B**（PERF）：debug_viz/lens_effects 遗漏中间 unbind，对齐 R196-B。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 517 处修复。
