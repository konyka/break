...
最近更新：**R201 VK SSS/FXAA/tonemap 独立 push 映射 — 修复 2 处** — **R201-A**（CORRECTNESS）：`u_sss_*`/`u_sssv_*` 未映射致 sw/sh=0 除零与皮下散射失效；补 sss_vk 偏移。**R201-B**（CORRECTNESS）：独立 `u_fxaa_threshold@8` 与 `u_tm_mode@16` 未映射；补 fxaa_vk/tonemap_vk 偏移。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 525 处修复。
