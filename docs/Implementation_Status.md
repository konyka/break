...
最近更新：**R102 ECS archetype edge 缓存** — **R102**（PERF）：`world_add_component`/`world_remove_component` 的目标 archetype 查找从 O(N) 线性扫描（`find_archetype` 遍历全部 archetype + hash 比对）降为 O(E) edge 查找（E = 每个 archetype 的 edge 数，典型 ≤ 组件类型数，个位数）。首次 add/remove 某 component 仍走 `find_archetype` 并缓存结果到 `edges_add[]`/`edges_remove[]`；后续相同 component 的转换直接用缓存的 `target` 指针，跳过类型数组构建+排序+hash 扫描。`ArchetypeEdge` 结构与 `edges_add/remove` 字段此前已定义但为桩（`ecs.c:45-47`），现已完整实现 `edge_lookup_add`/`edge_lookup_remove`/`edge_cache_add`/`edge_cache_remove` 四个辅助函数。`test_ecs` 新增 3 项 edge cache 测试（add 命中/remove 命中/50 实体多轮转换），**23/23** 通过。
