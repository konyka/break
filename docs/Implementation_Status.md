...
最近更新：**R172 staging 双缓冲 + Hi-Z 布局 + 粒子 emit + mipmap 生命周期 — 修复 5 处** — **R172-A**（CORRECTNESS）：双帧 Vulkan 下单一 staging 可与 GPU copy 并发；`rhi_frame_index` + gpucull/occlusion 双槽 staging。**R172-B**（CORRECTNESS）：Hi-Z mip 用 UNDEFINED 作 oldLayout 且末级未转可读；跟踪 `mip_layout[]`，生成后转 sampleable。**R172-C**（CORRECTNESS/PERF）：粒子 `emit_rate` 不限流且 VK push 陈旧；概率发射 + 每帧刷新 rate。**R172-D**（CORRECTNESS）：`force_level` 绕过预算。**R172-E**（ROBUSTNESS）：shutdown 取消在途 mip 请求。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 463 处修复。
