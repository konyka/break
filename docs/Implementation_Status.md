...
最近更新：**R200 VK color_grade/bloom push 映射 — 修复 2 处** — **R200-A**（CORRECTNESS）：独立 `u_cg_*` 未映射致调色饱和/对比度为 0；补 color_grade_vk 偏移（与 combined 布局分离）。**R200-B**（CORRECTNESS/PERF）：`u_threshold`/`u_direction`/`u_bloom_strength` 未映射致 bloom 不可见且 blur 空转；补 bloom_*_vk 偏移。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 523 处修复。
