...
最近更新：**R84 着色器效率优化(第二轮)：POM uniform门控 + SSGI循环不变量提升 + shadow_test提升 + F_Schlick pow→mul** — **R84-1**（HIGH GPU）：POM（视差遮挡映射）每片段无条件执行 18 次纹理采样，即使材质无高度图（fallback normal map .b=1.0 导致循环跑满）。添加 `u_pom_enabled` uniform 门控，无高度图时直接使用原始 UV。GL 变体添加 uniform 声明，VK 变体添加到 push constant 块（offset 228，总 232 字节 < 256 限制）+ rhi_vk.c 映射 + main.c bind_material 设置。**R84-2**（MEDIUM GPU）：SSGI 着色器中 `tangent_to_world` 函数在 16 次循环内调用，但 `up`/`tangent`/`bitangent` 仅依赖 `normal`（循环不变）。提升到循环外，同时发现 `normalize()` 冗余（tangent/bitangent 正交，组合长度已为 1）。**R84-3**（MEDIUM GPU）：`shadow_test()` 在方向光循环内调用但不依赖循环变量。提升到 4 个着色器的循环外。**R84-4**（LOW GPU）：`F_Schlick`/`fresnel_schlick` 中 `pow(x, 5.0)` 替换为 `x*x*x*x*x`（4 次乘法 vs 1 次 exp+log），影响 6 个着色器。**回归**：全部 **23/23** 测试通过。
此前：**R83 着色器效率优化：聚类Z切片log2替换 + 体积雾循环不变量提升 + DOF const数组 + AABB上传移至init** — **R83-1**（HIGH GPU）：6 个着色器中聚类 Z 切片查找用 24 次 `pow()` 线性扫描，数学等价替换为 1 次 `log2`：`z <= 24 * log2(ld/near) / log2(far/near)`。影响前向 PBR + 延迟光照两条主渲染路径，1080p 下每帧消除 ~48M 次超越函数调用。**R83-2**（MEDIUM GPU）：体积雾着色器中 `texture(u_vol_shadow, vUV)` 在 16 次迭代循环内执行但 vUV 不变，15 次冗余纹理采样提升到循环外；VK 变体额外移除死代码 `inverse(u_vol_view)`。**R83-3**（MEDIUM GPU）：DOF 着色器中 `hex_disk` 数组值仅依赖编译期常量 `RINGS=3`/`SAMPLES_PER_RING=6`，但每片段重计算 18 次 `cos`/`sin`/`normalize`/`dot`。替换为 `const` 数组预计算字面量。**R83-4**（LOW CPU）：R82-2 缓存的 AABB 数据每帧仍通过 `glBufferSubData` 上传相同静态数据到 GPU。上传移至 init，每帧仅保留 dispatch（Hi-Z 每帧变化）。**回归**：全部 **23/23** 测试通过。
...
| 统一剔除(unified) | 部分→Hi-Z(R1,R11,R12,R16) | R1: `unified_cull.comp` 单 pass 视锥+压缩。**R12**: 阴影/点光默认 unified。**R16**: unified 可选 Hi-Z 球体测试（上一帧金字塔，1 帧延迟）；CSM/点光 unified 传入 `occ_sys.hi_z_texture`。遗留：前向仍 per-mat compact；Hi-Z 未与独立 occ pass 合并为单 dispatch |
...
| 遮挡剔除 Hi-Z | 桩→部分(R11) | ~~`occlusion_cull.c` 结果从未被消费~~ **R11 已接线**：Hi-Z compute + 1 帧延迟 readback 结果经 `node_occ_visible` 驱动前向 indirect(`vis_flags &= occ`)与 CPU frustum 回退(跳过被挡节点)；默认开启(`BREAK_OCCLUSION=0` 可关)。遗留：Hi-Z 尚未并入 unified_cull 单 pass；阴影 pass 仍仅视锥剔除 |
...
| 延迟渲染 | 部分(R13,R16) | **R13**：G-Buffer + 全屏光照接 cluster/CSM/IBL。**R16**：MRT RT3 velocity（NDC motion）；TAA 延迟路径采样。**遗留**：前向无 velocity；点光 cubemap 阴影未接 deferred |
...
| 模块 | 状态 | 证据 / 说明 |
|------|------|-------------|
| ECS | 部分→补齐 system 调度(R6) | `ecs.c` archetype+chunk 完整。R6: 新增 `ecs_system.{h,c}` —— `EcsChunkView`+`ecs_chunk_column`(SoA 列)/`ecs_chunk_entity_ids`、`ecs_parallel_for`(每非空 chunk 一 task，`ts==NULL` 串行)、`EcsScheduler` 系统按注册序执行(系统间串行避免列写竞争、chunk 内并行)；`main.c` 物理→Transform 同步迁入 `sys_sync_transform_from_physics` 经 `tasks` 并行。`test_ecs_system` 5 项。遗留：`edges_add/remove` 桩(`ecs.c:45-47`)；查询仍仅 AND 无 Exclude/Optional |
...
| Animation | 部分 | 单 clip + GPU skin 可用；blend/IK 已实现但 demo 未接(`animation.c:208-436`)；事件回调存储但 evaluate 不触发 |
...
| Async Loader | 部分 | 真异步线程；`priority` 字段未生效；无解码管线 |
...
| VFS + packer | 部分 | 目录挂载 + .pak 只读；packer 仅 POSIX |
