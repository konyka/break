...
最近更新：**R233 cull 近平面 + GL shadow compute 后重绑 — 修复 2 处** — **R233-A**（CORRECTNESS）：legacy `cull.comp` 近裁剪仍用 NDC z=0，R212 只修了 unified；改为 -1。**R233-B**（CORRECTNESS）：GL compute `glUseProgram` 覆盖 graphics；shadow unified/legacy 间接绘制前重绑 depth pipeline。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 589 处修复。
