...
最近更新：**R131 VK VkResult 返回值检查 + fopen/fclose 配对审计** — 对 VK 后端全量扫描 vkCreate*/vkAllocateMemory/vkBindMemory/vkMapMemory 调用，发现 19 处未检查 VkResult 返回值。初始化路径 13 处（vk_create_swapchain 2 + vk_create_depth 4 + vk_create_framebuffers 1 + vk_create_render_pass 1 + vk_init 5），失败时后续 VK 函数接收 VK_NULL_HANDLE 导致 UB；资源创建路径 6 处（rhi_buffer_create 3 + rhi_texture_create 3），失败时清理已创建资源并返回 RHI_HANDLE_NULL。同时完成 fopen/fclose 配对审计（25+ 处全安全）和信号处理审计（无 Unix signal handler）。VK（ENGINE_VULKAN=ON）+ GL 构建成功。23/23 测试通过。VK VkResult 检查总计 19 处已修复。
此前：**R130 VK 初始化路径 calloc NULL 检查 + realloc/VLA/alloca/va_end 全量审计** — R129 标记为“已知限制”的 VK 初始化路径 8 处 calloc 全部添加 NULL 检查。同时完成 realloc（9 处全安全）、VLA（0）、alloca（0）、va_start/va_end（3:3）、memset/memcpy sizeof（0）全量审计。23/23 测试通过。calloc/malloc NULL 检查总计 50 处已修复，零已知限制。
