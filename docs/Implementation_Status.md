...
最近更新：**R215 GL 点阴影 COMPARE 关闭 + VK 点阴影 Z remap — 修复 2 处** — **R215-A**（CORRECTNESS）：GL 点阴影 cube 开了 `COMPARE_REF_TO_TEXTURE`，着色器却用 `samplerCube`+`.r` 手动比较，采样未定义；改为 `GL_NONE`。**R215-B**（CORRECTNESS）：`point_shadow_depth_vk.vert` 缺 OpenGL→Vulkan `clip.z` remap，近半锥体被裁掉；对齐 depth_only。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 553 处修复。
