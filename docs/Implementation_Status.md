...
最近更新：**R177 TaskWaitLink OOM 回滚 + copy_buffer 屏障 — 修复 2 处** — **R177-A**（CORRECTNESS）：`task_submit_dep` 在 `TaskWaitLink` malloc 失败时 `continue` 欠计 dep，子任务提前跑；改为回滚已挂 waiter 并返回 INVALID。**R177-B**（CORRECTNESS）：`rhi_cmd_copy_buffer` 无 suspend/transfer 屏障；VK 补 suspend+barrier，GL 补 SSBO→COPY 可见性。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 477 处修复。
