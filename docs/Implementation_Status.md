...
最近更新：**R179 粒子 live Push 整块上传 + compute 采样布局 — 修复 2 处** — **R179-A**（CORRECTNESS）：VK 粒子仍依赖陈旧 `_push_template` 且 `set_uniform_mat4` 只拷 64B；每帧从 live `ps->*` 组装 80B，经 `rhi_cmd_set_uniform_bytes` 一次上传。**R179-B**（CORRECTNESS）：`rhi_cmd_bind_texture_compute` 假定全链已是 READ_ONLY；Hi-Z 写后可能仍为 GENERAL；按 mip 转换到 SHADER_READ_ONLY 再采样。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 481 处修复。
