...
最近更新：**R86 关键bug修复 + 粒子GPU回读消除 + VBO/IBO绑定缓存 + sun_color缓存** — **R86-1**（CRITICAL）：R85 引入的 blinn_phong_clustered 平方链错误——`spec = NdH * NdH`（初始 NdH^2）后 5 次 `spec *= spec` 计算的是 NdH^64 而非 NdH^32，导致镜面高光过窄。修复为 `spec = NdH`（初始 NdH^1）后 5 次平方 = NdH^32。影响 4 处（2 着色器 × 各 2 循环）。**R86-2**（HIGH CPU/GPU）：`particles_render` 每帧通过 `rhi_buffer_map`（`glMapBufferRange(GL_MAP_READ_BIT)`）将存活粒子数从 GPU 回读到 CPU，造成 CPU-GPU 管线停顿。但顶点着色器已有早退保护（`if (P_INST >= draw_count) return`），回读完全多余。移除回读，直接绘制 `PARTICLES_MAX` 实例。**R86-3**（MEDIUM CPU）：`rhi_cmd_bind_vertex_buffer` 和 `rhi_cmd_bind_index_buffer` 无缓存，与引擎中所有其他 GL 状态缓存不一致（纹理/FBO/VAO/SSBO 等均已缓存）。添加 `g_gl_bound_vbo`/`g_gl_bound_ibo` 文件作用域缓存，VAO 变化时同步失效。**R86-4**（LOW CPU）：`sun_color` 和 `ambient_col` 每帧重计算但仅依赖 `sun_elevation` 和 `ambient_mult`，当 TOD 循环关闭时为冗余。纳入与 `cached_sun_dir` 相同的缓存条件块。**回归**：全部 **23/23** 测试通过。
此前：**R85 着色器效率优化(第三轮)：blinn_phong albedo缓存 + skybox循环不变量 + pow→mul批量替换 + F_Schlick去重** — R85-1 albedo缓存、R85-2 skybox循环不变量、R85-3 pow→mul批量替换(18着色器)、R85-4 F_Schlick去重。23/23 测试通过。
