...
最近更新：**R161 渲染器后处理与剩余模块深度审查** — 审查 18 个源文件覆盖渲染器后处理（lighting.c, deferred.c, water.c, terrain.c, particles.c, skybox.c, ssao.c, ssgi.c, ssr.c, taa.c, occlusion_cull.c, render_graph.c, lod.c）、动画（animation.c, skeleton.c）、物理角色控制（character.c）。发现并修复 2 处新问题：R161-A terrain.c grid_size=0 堆缓冲区溢出防护（grid_size=0 时 (grid_size-1) u32 下溢为 0xFFFFFFFF，索引生成循环运行 ~40 亿次写入远超 24 字节分配）、R161-B lod.c level_count > LOD_MAX_LEVELS 越界读防护（level_count > 4 时 thresholds_sq 和 meshes 数组越界读）。总计 416 处修复，23/23 测试通过。
