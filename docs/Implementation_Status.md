...
最近更新：**R130 VK 初始化路径 calloc NULL 检查 + realloc/VLA/alloca/va_end 全量审计** — R129 标记为“已知限制”的 VK 初始化路径 8 处 calloc 全部添加 NULL 检查：vk_init 中 3 处（layer properties/GPU list/queue list）LOG_FATAL + free(vk) + return false；vk_create_swapchain 中 4 处（present modes/swap images/swap views/render semaphores）LOG_FATAL + return；vk_create_framebuffers 中 1 处（framebuffers）LOG_FATAL + return。同时完成 realloc（9 处全安全）、VLA（0 处）、alloca（0 处）、va_start/va_end（3:3 完美配对）、memset/memcpy sizeof 指针错误（0 处）全量审计。VK（ENGINE_VULKAN=ON）+ GL 构建成功。23/23 测试通过。calloc/malloc NULL 检查总计：GL 16 + VK 34 = 50 处已修复，零已知限制。
此前：**R129 全量 calloc/malloc NULL 检查审计 — RHI 后端 7 处遗漏修复** — 对全代码库执行 `grep -rn 'calloc|malloc'` 全量审计，发现 R125+R128 遗漏了“辅助 FBO”函数中的 7 处 calloc：GL `rhi_cubemap_depth_fbo_create`（2 处）、VK `rhi_mrt_fbo_create`（3 处）、VK `rhi_cubemap_depth_fbo_create`（2 处）。修复：calloc 移到 rhi_alloc_slot 之前，检查 NULL，返回空结构体。全量审计确认 17 个子系统全安全。23/23 测试通过。
