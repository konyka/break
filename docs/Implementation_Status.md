...
最近更新：**R190 GL create 纹理缓存失效 + object_ssbo DEVICE_LOCAL — 修复 2 处** — **R190-A**（CORRECTNESS）：texture/offscreen/MRT/cubemap/shadow create 绕过 `gl_bind_tex_unit` 未清 `g_tex_cache`，resize 后误跳过 bind；补失效。**R190-B**（PERF）：`object_ssbo` 无 `initial_data` 留 HOST_VISIBLE，统一路径每帧 CS 穿 PCIe；零初始化进 DEVICE_LOCAL。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 503 处修复。
