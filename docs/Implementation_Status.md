...
最近更新：**R192 INDEX create 清 IBO 缓存 + light_grid DEVICE_LOCAL — 修复 2 处** — **R192-A**（CORRECTNESS）：INDEX `buffer_create` 解绑 ELEMENT_ARRAY 未清 `g_gl_bound_ibo`，后续 bind 误跳过。**R192-B**（PERF）：`light_grid` 因 TEXEL 被排除 DEVICE_LOCAL，GPU cull 每帧 ~1.5MB HOST_VISIBLE；允许 STORAGE|TEXEL + 零初始化。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 507 处修复。
