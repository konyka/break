...
最近更新：**R207 接触阴影视空间光向 + cmd push 回放 — 修复 2 处** — **R207-A**（CORRECTNESS）：`contact_shadow` 视空间步进却用世界空间 `sun_dir`，相机旋转时接触阴影方向错误；调用前用 view 3×3 变换。**R207-B**（CORRECTNESS）：`RENDER_CMD_PUSH_CONSTANTS` 回放为空操作；改为 `rhi_cmd_set_uniform_bytes`。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 537 处修复。
