...
最近更新：**R186 mega DEVICE_LOCAL 读回 + 静态 SSBO DEVICE_LOCAL — 修复 2 处** — **R186-A**（CORRECTNESS）：R181 后静态 mesh 为 DEVICE_LOCAL，mega bake 的 `rhi_buffer_map` 在独显失败并静默产出垃圾几何；新增 `rhi_buffer_read`（staging download），失败则 abort bake。**R186-B**（PERF）：`all_draws`/`draw_cmds`/`aabb` 静态 CPU 源 SSBO 零初始化进 DEVICE_LOCAL。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 495 处修复。
