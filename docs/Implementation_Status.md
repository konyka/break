...
最近更新：**R178 粒子 push 尾部上传 + GL frame_index — 修复 2 处** — **R178-A**（CORRECTNESS）：VK `particles_compute` 用 `set_uniform_mat4` 只拷 64B，80B Push 的 `lifetime_range` 未上传；补 `+76` 的 f32。**R178-B**（PERF）：GL `rhi_frame_index` 恒 0，双槽 staging 退化并每帧 map 同步；`frame_end` 递增。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 479 处修复。
