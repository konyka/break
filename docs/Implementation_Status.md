...
最近更新：**R110 着色器文件读取防御性加固** — **R110-1**（ROBUSTNESS）：`particles.c` 和 `water.c` 的 `read_file`/`file_read` 缺少 `ftell` 返回值检查和 `malloc` NULL 检查。当 `ftell` 返回 -1（错误）时，`malloc((usize)(-1)+1)=malloc(0)` 可能返回 NULL，随后 `fread(NULL,...)` 崩溃。当 `malloc` 对正常大小返回 NULL（内存不足）时同样崩溃。引擎中其他 23 个 `read_file` 实现均有完整防护，仅此 2 个遗漏。修复：添加 `sz < 0` 检查、`malloc` NULL 检查、`out_len` NULL 检查，与引擎其余实现统一。审查确认 10 个子系统（数学库、粒子 GPU 着色器、水面渲染、字体渲染器、调试 UI、渲染图、命令缓冲、VK swapchain、CSM/点光阴影、后期处理管线）无需修复。23/23 测试通过。
