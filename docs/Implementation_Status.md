...
最近更新：**R78 cubemap缓存修复R77回归 + skybox深度缓存 + 点阴影FBO解绑批处理** — **R78-1**：`rhi_cmd_bind_cubemap` 仍绕过纹理缓存直接调 `glActiveTexture`+`glBindTexture`+`glBindSampler`，不更新 `g_active_unit`/`g_tex_cache`/`g_sam_cache`——这是 R77-1 修复的同类 bug，但遗漏了此函数。**R77-1 回归**：R77-1 使 `rhi_cmd_bind_texel_buffers` 信任 `g_active_unit`，但 cubemap 绕过使其残留错误值，导致 TBO 可能绑定到错误纹理单元（蒙皮网格/延迟光照渲染错乱）。改为调用 `gl_bind_tex_unit`（已处理 cubemap target 检测）。同时消除 IBL cubemap 每帧 ~380 次冗余 GL 调用（64 材质组×2 cubemap×3 调用，IBL cubemap 在所有组中完全相同）。**R78-2**：`skybox_render` 直接调 `glDepthFunc(GL_LEQUAL/GL_LESS)` 不更新 `g_gl_depth_func` 缓存，导致后续 `rhi_cmd_set_depth_func_*` 误判缓存命中跳过 `glDepthFunc`——深度测试函数错误。改为调用 `rhi_cmd_set_depth_func_less_or_equal`/`rhi_cmd_set_depth_func_less`。**R78-3**：点光源阴影逐面调用 `point_shadow_render_end` 解绑 cubemap FBO，下一面立即重新绑定——每帧 ~96 次冗余 `glBindFramebuffer`+`glViewport` 调用（4 光源×6 面×2 对 bind/unbind+viewport）。改为所有面完成后统一解绑一次。**回归**：全部 **23/23** 测试通过。
