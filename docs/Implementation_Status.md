...
最近更新：**R188 GL param/program/VAO 销毁缓存失效 — 修复 2 处** — **R188-A**（CORRECTNESS）：R187 漏清 `g_gl_param_buf`，indirect count 缓冲 name 复用误跳过 bind。**R188-B**（CORRECTNESS）：`rhi_pipeline_destroy` 未失效 program/VAO 缓存，resize 重建后可能误跳过 UseProgram/BindVAO。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 499 处修复。
