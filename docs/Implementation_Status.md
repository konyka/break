...
最近更新：**R217 GL water/god_rays sampler binding + god rays 零强度跳过 — 修复 2 处** — **R217-A**（CORRECTNESS）：`water.frag` 的 `u_shadow_map` 无 `layout(binding=1)`，默认 unit 0，而 `water_render` 把阴影绑到 unit 1，采样残留地形 albedo 当深度。对齐 `water_vk.frag`/`terrain.frag`。**R217-B**（CORRECTNESS/PERF）：`god_rays.frag` 双 sampler 均默认 unit 0，深度遮挡失效；补 binding 0/1；`intensity<=0` 跳过 fullscreen 且 main 不切陈旧 FBO。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 557 处修复。
