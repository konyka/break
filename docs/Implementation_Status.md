...
最近更新：**R75 聚簇块裁剪跳过 + mega_mat_groups逆索引O(N×G)→O(N)** — **R75-1**：R74-2 跳过聚簇地形绘制后，聚簇块的裁剪/上传/绑定（`light_system_cull` ~98K CPU迭代/帧 + GPU upload + pipeline bind + ~10 uniform sets）变为纯浪费——确认 `lights.light_data_buf`/`light_grid_buf` 仅在聚簇块内部使用，前向路径后续无引用。光源填充（`clear` + `add_dir` + 32×`add_point`）保留——pt_shadows 下一帧读取 `lights.point_count`/`lights.point_lights`。删除未使用的 `clustered_set_point_shadow_uniforms` 函数。**R75-2**：`mega_mat_groups_draw`/`mega_mat_groups_indirect` 及两处 legacy 路径的 O(N×G) 线性扫描（每组遍历全部 draw command 查找属于该组的命令）替换为预构建逆索引 `group_cmd_offsets[]` + `group_cmd_list[]`。构建时单遍计数+前缀和+填充，总复杂度从 O(N×G) 降至 O(N)。每帧调用 12-17 次（CSM 4 级联 + 点光阴影 6 面/光 + 前向/延迟 1-2 次），以 N=16384、G=8 计，每帧减少约 160-220 万次迭代。构建阶段也从 O(N×G) 降至 O(N)。**回归**：全部 **23/23** 测试通过。
