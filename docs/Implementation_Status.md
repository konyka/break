...
最近更新：**R160 深层并发安全与整数算术溢出审查** — 审查 20+ 源文件聚焦线程安全、整数溢出、资源生命周期、缓冲区管理。发现并修复 2 处新问题：R160-A VFS PAK name_offset 越界读防护（恶意 PAK 文件 name_offset >= name_table_size 导致越界读 + strcmp 无 null 终止保障）、R160-B decode_pipeline out->size u32 截断防护（超大纹理解码后 usize→u32 截断）。总计 414 处修复，30/31 测试通过。
