...
最近更新：**R214 主通道 VK Z remap + bloom 零开销跳过 — 修复 2 处** — **R214-A**（CORRECTNESS）：主通道 VK 顶点（terrain/water/PBR/gbuffer/skinned/instanced/particle 等）缺 OpenGL→Vulkan `clip.z` remap，场景深度与后处理 `depth*2-1` 不一致；对齐 R213 CSM。**R214-B**（PERF）：`bloom_strength<=0` 仍跑 extract+blur+composite；`post_process_apply` 入口跳过。编译验证：Vulkan 100% + GL 100%。测试：VK/GL 各 30/30。总计 551 处修复。
