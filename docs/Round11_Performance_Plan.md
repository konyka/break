...
- [x] 前向/G-Buffer 仍走 CPU frustum+LOD+occ + per-material compact（材质切换需分组 bind）——**R441**：前向经纹理数组 + `mega_mat_arrays_draw` 单 bind 单 execute（`BREAK_MAT_INDIRECT` 默认开）；G-Buffer array 化列为第二阶段——**R442 已做**：`mega_mat_arrays_draw_gbuffer` 单 compact+bind+execute（MR 数组 pair 对齐），TEST 12 MRT 像素断言门禁
...
### [x] R363-B Space impulse：移出破损 `k` layout 大括号
### [x] R363-C `,` cam path：停录臂播放；`,` 置 `playing_path=true`
### [x] R363-D tornado/AA/trail/layout → KP_1..4 (306–309)；`t`/`h`/`j`/`k` 保留 filter/terrain/sun
### [x] R363-E water load：无 pipeline 则 `enabled=false`
