...
| 高级编辑器 | 物理行折叠支持严格包含嵌套、行号栏、wrap 增量缓存、行级 lexer 和受限 token 着色已实现；折叠状态持久化及完整 RTL token 绘制未实现 | 大文档单帧 O(n) 卡顿、折叠后索引失效、token 状态跨行污染 | 增加有界 YAML 折叠快照与 bidi run，继续保持 lexer/cache 单帧预算 |
