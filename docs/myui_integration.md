...
定时器内部按 `next_fire_ms` 和 ID 维护最小堆，并额外维护固定负载上限的开放寻址 ID 索引：
添加、重新入堆和到期条目处理为 O(log n)，`due_in_ms()` 为 O(1)，按 ID 删除为均摊
O(1) 定位加 O(log n) 堆调整。触发路径只处理已到期的根节点，不再随 timer 总数线性扫描；
索引扩容失败时添加事务回滚，不发布半初始化 timer。删除主要发生在控件生命周期路径，
