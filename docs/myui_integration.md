...
| 编辑器 | 代码折叠、增量语法高亮等高级编辑器能力未实现；text area 已提供可选物理行号栏，wrap cache 已采用前缀复用与 OOM-safe 后缀 candidate swap | 单独的 virtualized line model，限制单帧重排预算，避免把大文档编辑路径耦合到 widget 绘制 |
