...
NetRep peer 持久化要求每个 `.peer` 输出文件名完整容纳于内部 512-byte 路径缓冲；无法完整表示的目录/主机组合会在创建文件前报告失败，不会写入截断名称。导入会拒绝端口溢出及 NaN/Inf RTT 记录，避免无效文本污染运行时诊断状态。单文件基线读取会确认读取和关闭成功，失败时恢复原有运行时 peer 表；读取目录时同样只打开能完整组合的 `.peer` 与 `delta.log` 路径，避免把截断前缀文件解析为其他 peer，且目录无法打开或任一已打开 `.peer` 读取/关闭失败时不清空已加载的运行时 peer 基线，成功扫描后才以目录快照替换。全量 checkpoint、目录基线和增量日志均在全部写入及关闭成功后才报告成功；缺失增量日志仍可选，但已打开日志的 I/O 失败会恢复已应用的 delta，供调用者重试。
