...
`scene_state_load()` 将二进制存档中的全部浮点字段视为不可信输入：完整 `Camera`、太阳/曝光/渲染倍率、各版本刚体的位置/速度/质量/半尺寸/弹性与可选水位必须均为有限值；任一 NaN/Inf 都拒绝整个存档并恢复加载前运行时状态，避免污染渲染、物理或诊断计算。检查只在显式加载时线性执行，不增加帧内成本或分配。
