...
- 二进制与 JSON 导入及导出都限制为最多 64K `SceneNode`；保存器会在构造输出前拒绝超额图或计数非零但存储为空的场景，资源清单同样拒绝计数非零但 `meshes`/`materials` 存储为空，避免崩溃或成功写出随后必被加载器拒绝的文件；JSON 导入仍在扩容 staging 数组前拒绝超额节点，避免紧凑恶意文档造成无界堆增长
