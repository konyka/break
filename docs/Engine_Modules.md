...
纹理热重载也保留完整的受监视源路径；超过内部 255 字节存储上限的路径会在创建 watcher 前拒绝，避免后续重载命中截断后的其他文件。着色器热重载对顶点和片段路径采用相同约束，并让 watcher 使用已校验的内部副本。glTF 外部纹理 URI 与模型目录组合后也必须完整容纳内部路径缓冲；过长组合会跳过该纹理，不会加载截断名称的其他文件。
