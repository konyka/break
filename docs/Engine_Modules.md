...
- **Lua 文件路径与重载**: 需要热重载的脚本路径必须完整容纳在内部 256-byte（含终止符）字段；过长路径在加载前拒绝，避免首次加载与后续重载指向不同文件。候选脚本仅在编译和执行成功后提交 mtime，失败文件保持可在下一次轮询重试；若候选在执行中报错，其顶层全局变量和 hooks 会回滚，既有逻辑不被部分替换。
