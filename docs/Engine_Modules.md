...
目录与 PAK 挂载路径必须完整容纳内部 `VFS_MAX_PATH`（260 字节含终止符）字段；超长路径会在打开文件或占用挂载槽位前拒绝，避免后续资源查找或诊断身份落到截断路径。每次目录 mount 查找还要求根路径与相对资源路径的完整组合容纳内部 512-byte I/O 缓冲；不能组合的高优先级 mount 会跳过并继续尝试较低优先级挂载，绝不打开截断前缀文件。
...
纹理热重载也保留完整的受监视源路径；超过内部 255 字节存储上限的路径会在创建 watcher 前拒绝，避免后续重载命中截断后的其他文件。初始化失败的纹理/着色器热重载对象可安全调用 shutdown，不会把零值 watcher 误当成已打开的系统描述符。着色器热重载对顶点和片段路径采用相同约束，并让 watcher 使用已校验的内部副本。glTF 外部纹理 URI 与模型目录组合后也必须完整容纳内部路径缓冲；过长组合会跳过该纹理，不会加载截断名称的其他文件。
...
- 二进制与 JSON 导入及导出都限制为最多 64K `SceneNode`；BSCN 保存器在打开目标文件前以 `u64` 汇总完整格式大小、JSON 保存器在内存文档构造完成后检查实际字节数，二者都拒绝超过加载端 64 MiB 输入上限的输出，且拒绝超额图或计数非零但存储为空的场景。资源清单同样拒绝计数非零但 `meshes`/`materials` 存储为空、超过固定 256 个不同材质纹理句柄或资源条目总数 `u32` 溢出的场景，避免崩溃、静默丢失纹理引用或成功写出随后必被加载器拒绝的文件；JSON 导入仍在扩容 staging 数组前拒绝超额节点，避免紧凑恶意文档造成无界堆增长
- BSCN 资源内联描述符与场景节点矩阵的全部浮点在保存与加载时均要求有限；二进制或 JSON 节点矩阵中出现 NaN/Inf 会在写出或载入时拒绝整个候选并保留既有 `Scene`，包括调用方不请求节点输出而仅丢弃节点时，避免污染渲染变换与资源边界计算或让格式有效性依赖输出目标。BSCN probe/load 在任何解析前校验全部 chunk payload 均位于 chunk table 之后、处于文件范围内且彼此不重叠；`ENTITIES`、`COMPONENTS`、`RESOURCES`、`SCENE_NODES` 各只能出现一次，已解析的四类 chunk 必须精确消费各自完整 payload，不能把未解释尾部当作有效格式；未知/HIERARCHY chunk 仍为前向或历史兼容而整体跳过，但也不能重解释表字节。BSCN 实体 generation 与 JSON 中显式的 `gen` 均必须非零，避免把无效句柄静默替换成新建实体的 generation；v1 资源 flags 仅允许内联描述符的 bit 0，节点 flags 仅允许 `has_mesh`/`skinned` 的低两位；JSON 节点完整保存 BSCN 对称的 parent、mesh、material、skin_mesh、flags 与 local，显式空 `nodes` 数组也会替换既有图为零节点，已知键按 JSON 解码后的字符匹配以防 `\\uXXXX` 逃逸重复字段限制，组件 `data` 只可位于决定其 hex 长度的 `size` 之后，且 v1 组件记录必须含 `type`、`size`、`data` 三元组；安装本地兼容组件失败（如 ECS archetype 已达容量）会拒绝整个候选而不会静默丢弃 payload。顶层的 `version`、`entities`、`nodes`、实体内的 `gen`、`components`、组件内的 `type`、`size`、`data` 与节点内这些字段均最多出现一次，所有对象成员与节点数组元素必须以一个非尾随逗号分隔，未知字段的标量值必须是标准 JSON number、boolean 或 null，未知字符串必须使用合法 JSON 转义，未知复合值以固定 256 层非递归状态机验证嵌套 `{}`/`[]` 及其对象/数组语法，根对象结束后只允许空白而不能忽略追加值，缺失旧字段仍保留兼容默认值。每个 `ENTITIES` archetype key 中本版本可表示的 128 个组件 ID 均只能出现一次；JSON 的每个实体 `components` 数组同样至多 128 项，所有完整 `u32` 类型 ID（包括未知 ID）只能出现一次。`COMPONENTS` chunk 的类型记录数最多为 `ECS_MAX_COMPONENTS`，所有完整 `u32` 类型 ID（包括未知 ID）均只能出现一次，并在进入记录循环前校验固定头部字节数；每个类型记录的保存实体索引都必须唯一，即使类型未知或尺寸不匹配而被兼容跳过也不例外。已知且尺寸匹配的每个实例还必须对应实体 archetype 已声明的组件，不能由组件块隐式改变实体结构。对这种本地兼容类型，`ENTITIES` 声明与 `COMPONENTS` 实例必须构成一一对应：类型记录不能缺失、实例数必须精确相等；不同尺寸的同 ID 记录仍整体跳过以保持前向兼容。每种类型的实例数不得超过保存实体数且其索引/payload 最小字节数须完整可容纳，每个实例还必须引用 `ENTITIES` chunk 中的有效索引，拒绝不可能由写入端产生的膨胀、悬空或重复 owner 记录。对应检查仅在显式加载时执行，使用固定的 128 项声明计数、128 项类型列表和每类型复用的 64K 位实体位图；额外工作为 O(类型记录数平方 + 类型记录数 * 1024 + 实例数)，上限为 8,128 次类型比较，无堆分配或帧内成本
...
**特性**：
- 多挂载点优先级查找（后挂载的优先）
- PAK 打包格式（自定义，含 FNV-1a 哈希索引）；挂载前必须同时匹配 magic 与 `VFS_PAK_VERSION`，避免将不同条目布局的归档按当前格式错误解析
- PAK 名称表条目必须在声明的名称表范围内以 NUL 结束；未终止名称仅在挂载期跳过为查找 miss，不会借助内存分配的哨兵字节错误匹配路径。检查仅在构建索引时执行，不增加 `vfs_open()` 热路径开销或常驻分配
- 统一接口，上层无需区分目录/PAK 来源

**单元测试：** `tests/test_vfs.c` (33 测试用例，覆盖目录/PAK 挂载、文件读写、PAK 二进制格式、版本与未终止名称拒绝、挂载优先级、边界条件)
...
脚本文本的数值字面量属于不可信输入。加载器拒绝 `var`、`set`、`add` 与 `spawn` 中的 NaN/Inf；任一非法数值令候选加载失败并保留上一份有效脚本，避免非有限值传播到游戏逻辑。验证只发生在加载或热重载期间，不增加每帧 `script_call()` 的开销或常驻分配。
...
- **Lua 文件路径与重载**: 需要热重载的脚本路径必须完整容纳在内部 256-byte（含终止符）字段；过长路径在加载前拒绝，避免首次加载与后续重载指向不同文件。候选脚本仅在编译和执行成功后提交路径和 mtime，失败文件不会夺取上一份有效脚本的热重载身份，且保持可在下一次轮询重试；若候选在执行中报错，其顶层全局变量和 hooks 会回滚，既有逻辑不被部分替换。

**单元测试：** `tests/test_script.c` (18 测试用例)

覆盖：init/shutdown、set/get global、global 覆盖、缺失 global、多 global、文件加载+函数执行、不存在函数调用、不存在文件、失败替换状态保持、NaN/Inf 数值拒绝、注释忽略、每引擎热重载、空文件、操作数与文件大小上限。
...
`scene_state_save()` 仅在完整写入且关闭文件成功后返回成功，关闭阶段才发现的缓冲 I/O 错误会返回给调用方，避免将未持久化的运行时状态当作有效存档；写入前还会验证实际序列化的相机、全局和刚体浮点字段均有限，非法运行时值会在打开目标文件前失败，从而保留已有可恢复检查点。该校验仅在显式保存时按刚体数线性执行，不增加帧内成本或分配。

`scene_state_load()` 将二进制存档中的全部浮点字段视为不可信输入：完整 `Camera`、太阳/曝光/渲染倍率、各版本刚体的位置/速度/质量/半尺寸/弹性与可选水位必须均为有限值；任一 NaN/Inf 都拒绝整个存档并恢复加载前运行时状态，避免污染渲染、物理或诊断计算。水位开关固定编码为一个 `u8`，只接受 `0` 或 `1`，不把任意磁盘字节直接写入 C `bool`。检查只在显式加载时线性执行，不增加帧内成本或分配。
...
- 自动依赖推导（读访问依赖声明顺序中的最近生产者；若 pass 需读取并覆写同一逻辑资源，必须同时声明 read 与 write）
- 死路径剔除（未消费的输出对应的 pass 被自动裁剪）
- Kahn 拓扑排序保证执行顺序
- 纹理池生命周期别名（相同尺寸/格式的纹理可复用同一 GPU 资源）

**单元测试：** `tests/test_render_graph.c`（覆盖读写链、死路径零资源分配及高 fan-out 拓扑回归）

覆盖：创建/销毁、pass/resource 声明、依赖推导与执行顺序、同资源读写后处理链、死路径剔除与零 GPU 资源分配、执行回调、Reset 复用、资源查找、统计信息。
...
NetRep peer 持久化要求每个 `.peer` 输出文件名完整容纳于内部 512-byte 路径缓冲；无法完整表示的目录/主机组合会在创建文件前报告失败，不会写入截断名称。目录基线成功写完当前集合后，会清理陈旧的 `peer_*.peer` 文件，故轮转后已驱逐的 peer 不会在下一次加载时复活。导入会拒绝端口溢出及 NaN/Inf RTT 记录，避免无效文本污染运行时诊断状态。单文件基线读取会确认读取和关闭成功，失败时恢复原有运行时 peer 表；读取目录时同样只打开能完整组合的 `.peer` 与 `delta.log` 路径，避免把截断前缀文件解析为其他 peer，且目录无法打开或任一已打开 `.peer` 读取/关闭失败时不清空已加载的运行时 peer 基线，成功扫描后才以目录快照替换。全量 checkpoint、目录基线和增量日志均在全部写入及关闭成功后才报告成功；缺失增量日志仍可选，但已打开日志的 I/O 失败会恢复已应用的 delta，供调用者重试。
