...
- 二进制与 JSON 导入及导出都限制为最多 64K `SceneNode`；BSCN 保存器在打开目标文件前以 `u64` 汇总完整格式大小并拒绝超过加载端 64 MiB 输入上限的输出，且拒绝超额图或计数非零但存储为空的场景。资源清单同样拒绝计数非零但 `meshes`/`materials` 存储为空、超过固定 256 个不同材质纹理句柄或资源条目总数 `u32` 溢出的场景，避免崩溃、静默丢失纹理引用或成功写出随后必被加载器拒绝的文件；JSON 导入仍在扩容 staging 数组前拒绝超额节点，避免紧凑恶意文档造成无界堆增长
