...
目录挂载路径必须完整容纳内部 `VFS_MAX_PATH`（260 字节含终止符）字段；超长根路径会在占用挂载槽位前拒绝，避免后续相对资源查找落到截断目录。每次目录 mount 查找还要求根路径与相对资源路径的完整组合容纳内部 512-byte I/O 缓冲；不能组合的高优先级 mount 会跳过并继续尝试较低优先级挂载，绝不打开截断前缀文件。
...
**单元测试：** `tests/test_vfs.c` (32 测试用例，覆盖目录挂载、文件读写、PAK 二进制格式、版本与未终止名称拒绝、挂载优先级、边界条件)
