...
/* Combined tonemap + colour grade + cinematic in a single pass.
 * Order: chromatic aberration sampling -> tonemap (exposure + curve + gamma)
 *        -> colour grade (saturation/contrast/brightness/white balance)
 *        -> vignette + film grain. */
...
/* R271: 1x1 adapted-luminance target (matches tonemap_vk.frag u_tm_lum@1). */
layout(binding = 1) uniform sampler2D u_tm_lum;

layout(push_constant) uniform CombinedColorParams {
    layout(offset = 0)  float u_tm_exposure;
    layout(offset = 4)  float u_tm_gamma;
    layout(offset = 8)  int   u_tm_mode;
    layout(offset = 12) float u_cg_saturation;
    layout(offset = 16) float u_cg_contrast;
    layout(offset = 20) float u_cg_brightness;
    layout(offset = 24) float u_cg_temperature;
    layout(offset = 28) float u_cg_tint;
    layout(offset = 32) float u_cine_aberration;
    layout(offset = 36) float u_cine_vignette;
    layout(offset = 40) float u_cine_grain;
    layout(offset = 44) float u_cine_time;
    layout(offset = 48) float u_screen_w;
    layout(offset = 52) float u_screen_h;
};
...
void main() {
    ...
    /* Tonemap. R271: apply auto-exposure identically to tonemap_vk.frag so the
     * fused path matches the separate tonemap pass (was: fixed hdr *=
     * u_tm_exposure, silently ignoring the per-frame auto-exposure). */
