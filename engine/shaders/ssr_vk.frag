...
layout(push_constant) uniform SSRParams {
    layout(offset = 0)   mat4 u_ssr_proj;
    layout(offset = 64)  mat4 u_ssr_inv_proj;
    layout(offset = 128) mat4 u_ssr_view;
    layout(offset = 192) float u_ssr_sw;
    layout(offset = 196) float u_ssr_sh;
    layout(offset = 200) float u_ssr_max_steps;
    layout(offset = 204) float u_ssr_stride;
    layout(offset = 208) float u_ssr_thickness;
};
...
void main() {
    vec2 half_uv = vUV;
    float depth = texture(u_ssr_depth, half_uv).r;
    ...
    vec3 pos_vs = view_pos_from_depth(half_uv, depth);
    ...
    float step_count = u_ssr_max_steps;
    vec3 step_delta = ray_delta / step_count * u_ssr_stride;
    ...
    for (int i = 0; i < int(step_count); i++) {
        ...
        float ray_depth = ray.z;

        float diff = ray_depth - sample_depth;
    ...
    if (found) {
        ...
        fade = smoothstep(0.0, 0.1, edge_fade);
        fade *= max(dot(normal_vs, view_dir), 0.0);
    }
