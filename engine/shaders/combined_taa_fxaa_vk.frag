...
/* Combined TAA resolve + FXAA in a single pass.
 *
 * Pass 1 (TAA): neighbourhood-clamped temporal reprojection of the current
 * colour against the history texture.
 * Pass 2 (FXAA): edge-directed spatial AA applied to the temporally-resolved
 * colour.  Because the resolved colour is only available for the current
 * pixel, FXAA samples the *current* colour texture for its neighbourhood and
 * applies the computed sub-pixel offset to it, then blends toward the TAA
 * result.  This keeps everything in one draw while preserving both AA cues.
 */
...
layout(push_constant) uniform CombinedAAParams {
    layout(offset = 0)   mat4  u_taa_curr_vp;
    layout(offset = 64)  mat4  u_taa_prev_vp;
    layout(offset = 128) mat4  u_taa_inv_proj;
    layout(offset = 192) float u_screen_w;
    layout(offset = 196) float u_screen_h;
    layout(offset = 200) float u_taa_blend;
    layout(offset = 204) float u_taa_first_frame;
    layout(offset = 208) float u_fxaa_threshold;
    layout(offset = 212) float u_taa_use_velocity;
};
...
/* FXAA over the current-colour texture; returns the anti-aliased colour. */
...
void main() {
    ...
    /* Spatial AA on the current colour. */
    vec3 aa_color = fxaa(vUV, texel);

    /* Neighbourhood for temporal clamping (from the current colour). */
