...
void main() {
    ...
    for (int i = 0; i < KERNEL_SIZE; i++) {
        ...
        vec3 sample_pos = pos + (tangent * sample_dir.x + bitangent * sample_dir.y + normal * sample_dir.z) * u_ssao_radius;

        vec4 offset = u_ssao_proj * vec4(sample_pos, 1.0);
        offset.xy = offset.xy / offset.w * 0.5 + 0.5;

        float sample_depth = texture(u_depth, offset.xy).r;
        vec3 sample_world = reconstruct_pos(offset.xy, sample_depth);

        float range_check = smoothstep(0.0, 1.0, u_ssao_radius / abs(pos.z - sample_world.z));
        occlusion += (sample_world.z >= sample_pos.z + u_ssao_bias ? 1.0 : 0.0) * range_check;
    }
