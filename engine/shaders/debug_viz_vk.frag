...
layout(push_constant) uniform DebugVizParams {
    layout(offset = 0)  int   u_dv_mode;
    layout(offset = 4)  float u_dv_near;
    layout(offset = 8)  float u_dv_far;
    layout(offset = 12) float u_dv_split0;
    layout(offset = 16) float u_dv_split1;
    layout(offset = 20) float u_dv_split2;
    layout(offset = 24) float u_dv_split3;
};
...
void main() {
    ...
    if (u_dv_mode == 0) {
        ...
        float splits[4] = float[4](u_dv_split0, u_dv_split1, u_dv_split2, u_dv_split3);
