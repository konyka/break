...
re_status_t re_facts_justification_add(re_facts_t *facts, re_fact_id_t derived, re_string_t rule,
    ...
    if (facts->tms == NULL) {
        ...
        facts->tms->allocator = facts->allocator; facts->tms->items = NULL; facts->tms->count = 0u; facts->tms->capacity = 0u;
    }
    for (i = 0u; i < count; ++i) {
    ...
    if (tms->count == tms->capacity) {
        ...
        re_tms_justification_t *grown = re_realloc(&tms->allocator, tms->items, capacity * sizeof(*grown));
...
size_t re_facts_justification_count(const re_facts_t *facts, re_fact_id_t id) {
    ...
    for (i = 0u; i < facts->tms->count; ++i) if (id_equal(facts->tms->items[i].derived, id)) ++count;
    return count;
}
...
re_status_t re_facts_justification_remove(re_facts_t *facts, re_fact_id_t derived, re_string_t rule,
                                          const re_fact_id_t *premises, size_t count) {
    size_t i;
    if (facts == NULL || facts->tms == NULL || facts->transaction != NULL) return facts != NULL && facts->transaction != NULL ? re_facts_justification_remove(facts->transaction->staged, derived, rule, premises, count) : RE_STATUS_INVALID_ARGUMENT;
    for (i = 0u; i < facts->tms->count; ++i) if (id_equal(facts->tms->items[i].derived, derived) && justification_equal(&facts->tms->items[i], rule, premises, count)) {
        ...
        if (re_facts_justification_count(facts, derived) == 0u && valid_id(facts, derived)) return re_facts_retract(facts, derived);
        return RE_STATUS_OK;
    }
...
void re_tms_remove_premise(re_facts_t *facts, re_fact_id_t premise) {
    ...
    while (i < facts->tms->count) {
        ...
        if (!supported) { ++i; continue; }
        {
            ...
            if (re_facts_justification_count(facts, derived) == 0u && valid_id(facts, derived))
