...
typedef struct {
    ...
    /* R420: use the platform mutex type directly; task.c passes &ts->submit_mutex
     * to platform_mutex_* everywhere. */
    ...
    Task         *_task_block;
    /* R420: embedded platform mutex, shared by both Windows and POSIX paths. */
    PlatformMutex pool_mutex;
} TaskSystem;
