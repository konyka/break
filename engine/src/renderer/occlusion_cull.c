 ...
 *
 * R436: Segmented generation chain (was: one dispatch + one barrier per mip
 * level — 10 dispatches at 1280x720). Each dispatch now produces a chunk of
 * up to 4 consecutive mip levels in a single pass: every output texel is
 * reduced directly from the chunk input (2^(k+1) x 2^(k+1) block), so no
 * cross-workgroup synchronization is required and no atomic-spin SPD tricks
 * are needed. Chunks are separated by one memory barrier (chunk c+1 samples
 * the last mip written by chunk c). 10 levels -> 3 dispatches + 3 barriers.
 * The chunk size of 4 matches the RHI's 4 storage-image compute bindings.
 * External contract unchanged: same pyramid contents (equal or more
 * conservative at odd sizes), same 1-frame consumption latency, same
 * post-condition that the full pyramid is left sampleable.
 * ======================================================================== */
void occlusion_cull_generate_hi_z(OcclusionCullSystem *sys, RHICmdBuffer *cmd, RHITexture depth_buffer) {
    ...
    /* R436: reset per-call dispatch counter (perf observability). */
    sys->hi_z_dispatch_count = 0;

    /* Bind the Hi-Z generation pipeline */
    rhi_cmd_bind_pipeline(cmd, sys->hi_z_pipeline);

    /* Generate mip chunks of up to 4 levels each */
    for (u32 first = 0; first < sys->hi_z_levels; first += 4u) {
        u32 count = sys->hi_z_levels - first;
        if (count > 4u) count = 4u;

        u32 first_w = sys->hi_z_width  >> first;
        u32 first_h = sys->hi_z_height >> first;
        if (first_w < 1) first_w = 1;
        if (first_h < 1) first_h = 1;

        /* R436: Re-bind the pipeline per chunk. The VK RHI accumulates
         * storage-image binds into one descriptor set per pipeline bind;
         * re-binding starts a fresh set, so this chunk's descriptors never
         * update a set already referenced by the previous chunk's recorded
         * dispatch (which would be invalid usage / read-at-execution races).
         * The GPU-side vkCmdBindPipeline is skipped when unchanged (R89-1). */
        rhi_cmd_bind_pipeline(cmd, sys->hi_z_pipeline);

        /* Bind input: chunk 0 reads the depth buffer, later chunks read the
         * previous mip through a single-mip sampler view. Both must target
         * the compute sampler set (set 2). rhi_cmd_bind_texture would wrongly
         * target the graphics material set, leaving set 2 unbound -> GPU crash. */
        if (first == 0) {
            rhi_cmd_bind_texture_compute(cmd, depth_buffer, sys->hi_z_sampler, 0);
        } else {
            rhi_cmd_bind_texture_mip(cmd, sys->hi_z_texture, sys->hi_z_sampler, 0, first - 1u);
        }

        /* Bind outputs: the chunk's mips as storage images, units 0..3.
         * Unused tail slots (count < 4) are padded with the last valid mip
         * view so all 4 descriptors are valid; the shader never writes them
         * (guarded by the mip count in the push constants). */
        for (u32 k = 0; k < 4u; k++) {
            u32 mip = first + k;
            if (mip > sys->hi_z_levels - 1u) mip = sys->hi_z_levels - 1u;
            rhi_cmd_bind_image_texture(cmd, sys->hi_z_texture, k, mip, true);
        }

        /* Push constants: xy = size of the chunk's first output mip,
         * z = number of output mips in this chunk. */
        if (sys->_loc_hi_z_output_size >= 0) {
            rhi_cmd_set_uniform_vec4(cmd, sys->_loc_hi_z_output_size,
                                     (f32)first_w, (f32)first_h, (f32)count, 0.0f);
        }

        /* One 1D dispatch covering every output texel of the chunk; the
         * shader maps linear thread id -> (relative mip, texel) using the
         * same size math as below. */
        u32 total = 0;
        for (u32 k = 0; k < count; k++) {
            u32 w = first_w >> k, h = first_h >> k;
            if (w < 1) w = 1;
            if (h < 1) h = 1;
            total += w * h;
        }
        rhi_cmd_dispatch(cmd, (total + 63u) / 64u, 1, 1);
        sys->hi_z_dispatch_count++; /* R436 */

        /* Memory barrier between chunks (next chunk samples this output) */
...
u32 occlusion_cull_hiz_dispatch_count(const OcclusionCullSystem *sys) {
    /* R436: perf observability for the segmented Hi-Z chain. */
    return sys ? sys->hi_z_dispatch_count : 0u;
}
