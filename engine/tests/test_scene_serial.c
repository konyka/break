...
/* Defined v1 chunks have no extension trailer.  Only unknown chunk types are
 * forward-compatible skips; otherwise a malformed payload is accepted as a
 * valid prefix and its bytes are silently ignored. */
TEST(load_binary_rejects_known_chunk_trailing_bytes)
{
    char path[64];
    test_tmp(path, sizeof path, "test_bscn_known_chunk_trailer.bscn");
    const u32 entity_chunk[] = { 1u, 1u, 0u, 0xDEADBEEFu };
...
{
    char path[64];
    test_tmp(path, sizeof path, "test_bscn_undeclared_component.bscn");
    const u32 entity_chunk[] = { 1u, 1u, 0u };
    const u32 component_chunk[] = { 1u, 1u, sizeof(u32), 1u, 0u, 0x12345678u };
    const u32 base = (u32)sizeof(BscnHeader) + 2u * (u32)sizeof(BscnChunkEntry);
    BscnHeader header = { .magic = BSCN_MAGIC, .version = BSCN_VERSION, .chunk_count = 2 };
    BscnChunkEntry table[2] = {
        { .type = BSCN_CHUNK_ENTITIES, .offset = base, .size = sizeof(entity_chunk) },
...
/* ENTITIES declares the complete local archetype.  The writer therefore emits
 * one matching COMPONENTS instance for every declared, compatible component. */
TEST(load_binary_rejects_declared_component_without_instance)
{
    char path[64];
    test_tmp(path, sizeof path, "test_bscn_missing_component_instance.bscn");
    const u32 entity_chunk[] = { 1u, 1u, 1u, 1u };
    const u32 component_chunk[] = { 1u, 1u, sizeof(u32), 0u };
    const u32 base = (u32)sizeof(BscnHeader) + 2u * (u32)sizeof(BscnChunkEntry);
    BscnHeader header = { .magic = BSCN_MAGIC, .version = BSCN_VERSION, .chunk_count = 2 };
    BscnChunkEntry table[2] = {
        { .type = BSCN_CHUNK_ENTITIES, .offset = base, .size = sizeof(entity_chunk) },
...
TEST(load_binary_rejects_declared_component_without_type_record)
{
    char path[64];
    test_tmp(path, sizeof path, "test_bscn_missing_component_record.bscn");
    const u32 entity_chunk[] = { 1u, 1u, 1u, 1u };
    const u32 component_chunk[] = { 0u };
    const u32 base = (u32)sizeof(BscnHeader) + 2u * (u32)sizeof(BscnChunkEntry);
    BscnHeader header = { .magic = BSCN_MAGIC, .version = BSCN_VERSION, .chunk_count = 2 };
    BscnChunkEntry table[2] = {
        { .type = BSCN_CHUNK_ENTITIES, .offset = base, .size = sizeof(entity_chunk) },
...
TEST(load_binary_rejects_duplicate_component_instance_owner)
{
    char path[64];
    test_tmp(path, sizeof path, "test_bscn_duplicate_component_owner.bscn");
    const u32 entity_chunk[] = { 2u, 1u, 1u, 1u, 1u, 1u, 1u };
    const u32 component_chunk[] = {
        1u, 1u, sizeof(u32), 2u, 0u, 0x11111111u, 0u, 0x22222222u
    };
    const u32 base = (u32)sizeof(BscnHeader) + 2u * (u32)sizeof(BscnChunkEntry);
    BscnHeader header = { .magic = BSCN_MAGIC, .version = BSCN_VERSION, .chunk_count = 2 };
    BscnChunkEntry table[2] = {
...
TEST(load_json_rejects_incomplete_component_record)
{
    char path[64];
    test_tmp(path, sizeof path, "test_json_incomplete_component.json");
    const char *docs[] = {
        "{\"version\":1,\"entities\":[{\"components\":[{\"size\":4,\"data\":\"00000000\"}]}]}",
        "{\"version\":1,\"entities\":[{\"components\":[{\"type\":1,\"data\":\"\"}]}]}",
        "{\"version\":1,\"entities\":[{\"components\":[{\"type\":1,\"size\":4}]}]}"
    };
...
TEST(load_json_rejects_mismatched_unknown_compound_delimiters)
{
    ...
    const char *valid =
        "{\"version\":1,\"future\":\"quote:\\\" slash:\\\\ unicode:\\u4e2d\"}";
    FILE *fp = fopen(path, "wb");
    ASSERT_NOT_NULL(fp);
    ASSERT_TRUE(fwrite(valid, 1, strlen(valid), fp) == strlen(valid));
    ASSERT_TRUE(fclose(fp) == 0);
...
TEST(load_json_rejects_invalid_unknown_strings)
{
    char path[64];
    test_tmp(path, sizeof path, "test_json_invalid_unknown_string.json");
    const char *docs[] = {
        "{\"version\":1,\"future\":\"\\q\"}",
        "{\"version\":1,\"future\":\"\\u12G4\"}",
        "{\"version\":1,\"future\":\"line\nfeed\"}"
    };
...
TEST(save_rejects_nodes_above_load_limit)
{
    char binary_path[64], json_path[64];
    test_tmp(binary_path, sizeof binary_path, "test_save_node_limit.bscn");
    test_tmp(json_path, sizeof json_path, "test_save_node_limit.json");
    World *w = world_create();
    Scene scene; memset(&scene, 0, sizeof(scene));
    ASSERT_NOT_NULL(w);
    scene.node_count = 64u * 1024u + 1u;
    scene.nodes = (SceneNode *)calloc(scene.node_count, sizeof(SceneNode));
    ASSERT_NOT_NULL(scene.nodes);
...
TEST(save_binary_rejects_files_above_load_limit)
{
    char path[64];
    test_tmp(path, sizeof path, "test_save_file_limit.bscn");
    World *w = world_create();
    ASSERT_NOT_NULL(w);
    const u32 component_size = BSCN_MAX_FILE_BYTES;
    world_register_component(w, 1u, component_size);
    Entity entity = world_create_entity(w);
    ASSERT_TRUE(entity_valid(entity));
    ASSERT_NOT_NULL(world_add_component(w, entity, 1u));

    ASSERT_FALSE(scene_save_binary(w, NULL, path, NULL));

    world_destroy(w);
    remove(path);
}

TEST(save_prefab_rejects_files_above_load_limit)
{
    char path[64];
    test_tmp(path, sizeof path, "test_save_prefab_file_limit.bscn");
    World *w = world_create();
    ASSERT_NOT_NULL(w);
    const u32 component_size = BSCN_MAX_FILE_BYTES;
    world_register_component(w, 1u, component_size);
    ...
    ASSERT_FALSE(scene_save_prefab(w, &entity, 1u, path));
...
TEST_MAIN_BEGIN()
    ...
    RUN_TEST(load_json_rejects_invalid_unknown_strings);
